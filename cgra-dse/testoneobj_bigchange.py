...
params = [
    ...
    {'name' : 'num_itrack_per_ipin', 'type' : 'int', 'lb' : 2, 'ub' : 8,'step':2},
    {'name' : 'num_otrack_per_opin', 'type' : 'int', 'lb' : 2, 'ub' : 8,'step':2},
    {'name' : 'num_ipin_per_opin', 'type' : 'int', 'lb' : 2, 'ub' : 6,'step':2},
...
def addGIB(row,col,diag_iopin_connect,fclist):
    gib=[]
    for i in range(row+1):
        gib.append([])
        for j in range(col+1):
            gib[i].append({"diag_iopin_connect" : diag_iopin_connect,"fclist" : fclist})
...
for i in range(it):
    ...
    spec.update(**op)
    fclist=[]
    fclist.append(spec["connect_flexibility"]["num_itrack_per_ipin"])
    fclist.append(spec["connect_flexibility"]["num_otrack_per_opin"])
    fclist.append(spec["connect_flexibility"]["num_ipin_per_opin"])
    #op hete peocess
    row=spec["num_row"]
    col=spec["num_colum"]
    maxDelay=spec["max_delay"]
    diag_iopin_connect=spec["diag_iopin_connect"]
    GIB={"gibs":addGIB(row,col,diag_iopin_connect,fclist)}
    spec.update(**GIB)
    ...
    #run CGRA modeling and generation and dump CGRA.v to cgra-mg to run ppa
        ...
        y.append(restoreResult[0]/840000)
        ...
        #     keyAndSignleValue.append(formResult(spec))
        # else:
        #     if(restoreResult[0]<= 1.2*opt.y.min()):
        #         y.append(restoreResult[0]/840000)
        #         keyAndSignleValue.append(formResult(spec))
