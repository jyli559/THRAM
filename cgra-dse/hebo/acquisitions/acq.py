...
class MACE(Acquisition):
    ...
    def eval(self, x : torch.FloatTensor, xe : torch.LongTensor) -> torch.FloatTensor:
        ...
        """
        with torch.no_grad():
            py, ps2   = self.model.predict(x, xe)  #Return py and ps2 where py is the mean and ps2 predictive variance.
            noise     = np.sqrt(2.0) * self.model.noise.sqrt()
            ps        = ps2.sqrt().clamp(min = torch.finfo(ps2.dtype).eps)#To aviod ps to be zero and raiseerror in normed use eps to replace 0
            #lcb       = (py + noise * torch.randn(py.shape)) - self.kappa * ps
            lcb       = (py + noise * torch.randn(py.shape)-inf) - self.kappa * ps
            #print(lcb)
            normed    = ((self.tau - self.eps - py - noise * torch.randn(py.shape)) / ps)
            dist      = Normal(0., 1.) # 0 1 normal distribution
            log_phi   = dist.log_prob(normed)
            Phi       = dist.cdf(normed)
            PI        = Phi
            PI        = PI.add(-inf)
            #print(PI)
            EI        = ps * (Phi * normed +  log_phi.exp()-inf) 
