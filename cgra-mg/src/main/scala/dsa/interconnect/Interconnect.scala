...
//      println(ins(0))
//      println(trackDirections)
//      println(trackDirections(ins(0)))
      !((ins(0) <= 3 && trackDirections(ins(0)) == 0) || (ins(2) <= 3 && trackDirections(ins(2)) == 0))
    } )
  }
...
//object regest extends App{
//  (new chisel3.stage.ChiselStage).emitVerilog(new Reg(UInt(32.W)).suggestName("Mux1"),args)
//}
...
//   Map(num_otrack_per_opin -> 2, num_itrack_per_ipin -> 2, num_ipin_per_opin -> 4)
   val attrs: mutable.Map[String, Any] = mutable.Map(
     ...
     "track_directions, " -> ListBuffer(1,1,1,1)
   )
