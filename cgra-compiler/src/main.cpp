...
int main(int argc, char* argv[]) {
    ...
    for(auto& dfg_fn : dfg_fns){
        ...
        bool succeed = mapper.execute(dumpConfig, dumpMappedViz, resultDir);
        if(!succeed){
            //break;
            continue;
        }
        numSucceed++;
        float usagePE = (float)numNodes/numADGNodes;
    ...
    if(numSucceed == numDfg){
        ...
        fout2.close();
        std::ofstream fout3("mappingFailureRate.txt");
        fout3 << 0 << std::endl;
        fout3.close();
        std::cout << "The average usage of PE is:  " << avgPEUsage << std::endl;
        std::cout << "The average best max latency is:  " << avgLatency << std::endl;     
        std::cout << "The mapping failure rate is:  " << 0 << std::endl;
    }else if(numSucceed != 0){
        float mappingFailureRate = 1 - (float)numSucceed/(float)numDfg;
        std::cout << "Fail to map the DFG:  " << dfg_fns[numSucceed] << std::endl;
        std::cout << "Succeed to map : "<< numSucceed << " DFGs" << std::endl;
        float avgPEUsage = std::accumulate(storePEusage.begin(), storePEusage.end(), 0.0) / storePEusage.size();
        float avgLatency = std::accumulate(bestLatency.begin(), bestLatency.end(), 0) / (float)bestLatency.size();
        ...
        fout2.close();
        std::ofstream fout3("mappingFailureRate.txt");
        fout3 << mappingFailureRate << std::endl;
        fout3.close();
        std::cout << "The average usage of PE is:  " << avgPEUsage << std::endl;
        std::cout << "The average best max latency is:  " << avgLatency << std::endl;
        std::cout << "The mapping failure rate is:  " << mappingFailureRate << std::endl;

    }
    else{
        std::cout << "Fail to map  all the DFGs:  " << std::endl;
        exit(1);
    }
    std::cout << "=============================================\n";
