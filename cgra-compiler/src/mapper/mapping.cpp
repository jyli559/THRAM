...
void Mapping::unmapDfgNode(DFGNode* dfgNode){
    ...
    // std::cout << "Unmap DFG node " << dfgNode->name() << " from ADG node " << adgNode->id() << std::endl;
    // if(adgNode){
    //     _adgNodeAttr.erase(adgNode->id());
    // }
        ...
        for(int outPortIdx : outPortIdxs){
            for(auto& elem : adgNode->output(outPortIdx)){
                ...
                if((dstNode != nullptr) && (dstNode->id() == nextNodeId) && dstPortRange.count(nextSrcPort)){ // get to the dstNode                    
                    ...
                    continue;
                }else if((dstNode == nullptr) && nextNodeType == "OB"){ // route to OB
    ...
    while(!nodeQue.empty()){
        ...
        for(int inPortIdx : inPortIdxs){
            ...
            if((srcNode != nullptr) && (srcNode->id() == nextNodeId) && srcNodeOutPortIdx == nextDstPort){ // get to the srcNode                    
                ...
                continue;
            }else if(srcNode == nullptr && nextNodeType == "IB"){ // route to IB
...
std::set<int> Mapping::availDstPorts(DFGEdge* edge, ADGNode* dstNode){
    ...
    for(int opIdx : opIdxs){
        ...
        for(int inPort : inPorts){ // input ports connected to the operand with index of opIdx
            if(isAdgNodeInPortUsed(dstNode->id(), inPort)){
                operandUsed = true;
                break;
            }
        }
...
// get currently available delay cycles in the GPE node according to the mapped DFG node
int Mapping::getAvailDelay(GPENode* gpeNode, DFGNode* dfgNode){
    int maxDelay = gpeNode->maxDelay();
    int numOp = dfgNode->inputs().size();
    int availDelay;
    if(_fuDelayAttr.count(gpeNode->id())){
        auto& attr = _fuDelayAttr[gpeNode->id()];
...
void Mapping::latencySchedule(){
    ...
    while(dfgNode){ // until getting to the input port
        ...
        _fuDelayAttr[gpeNode->id()].delayUsed[inPort] = 0;
        _dfgNodeAttr[nodeId].maxLat = inPortLat; // input port max latency 
        _dfgNodeAttr[nodeId].minLat = std::max(inPortLat - getAvailDelay(gpeNode, dfgNode), 0); // input port min latency 
        dfgNode = srcNode;
    }
    ...
    while(!unscheduledNodes.empty()){
        for(auto iter = unscheduledNodes.begin(); iter != unscheduledNodes.end();){
            ...
            _dfgNodeAttr[nodeId].minLat = std::max(_dfgNodeAttr[nodeId].maxLat - getAvailDelay(gpeNode, dfgNode), 0); // input port min latency 
            ...
            for(auto& outsPerPort : dfgNode->outputEdges()){
                for(auto& eid : outsPerPort.second){
                    ...
                    GPENode* dstGpeNode = dynamic_cast<GPENode*>(_dfgNodeAttr[dstNodeId].adgNode); // mapped GPE node
                    int delayUsed = std::min(delayRequired, getAvailDelay(dstGpeNode, dstNode));  
                    _fuDelayAttr[dstGpeNode->id()].delayUsed[inPort] = delayUsed;
                    _fuDelayAttr[dstGpeNode->id()].totalDelayUsed += delayUsed;
                    _dfgNodeAttr[dstNodeId].minLat = std::max(_dfgNodeAttr[dstNodeId].maxLat - getAvailDelay(dstGpeNode, dstNode), 0); // input port min latency                     
                }
            }
        }
    }
...
void Mapping::calEdgeLatVio(){
    ...
    for(DFGNode* node : _dfg->topoNodes()){        
        ...
        GPENode* gpeNode = dynamic_cast<GPENode*>(_dfgNodeAttr[nodeId].adgNode);
        int gpeNodeId = gpeNode->id();
        auto& gpeDelayAttr = _fuDelayAttr[gpeNodeId];
        auto& delayUsed = gpeDelayAttr.delayUsed;
        for(auto& elem : node->inputEdges()){
            int eid = elem.second;
            int routeLat = _dfgEdgeAttr[eid].lat; // latNoDelay;
            int srcNodeId = _dfg->edge(eid)->srcId();
            ...
            if(!delayUsed.count(elem.first)){ // not assign delay for this port
                assignedDelay = std::min(getAvailDelay(gpeNode, node), requiredDelay);
                delayUsed[elem.first] = assignedDelay;
                gpeDelayAttr.totalDelayUsed += assignedDelay;
                _dfgNodeAttr[nodeId].minLat = std::max(_dfgNodeAttr[nodeId].maxLat - getAvailDelay(gpeNode, node), 0); // input port min latency 
            }else{
