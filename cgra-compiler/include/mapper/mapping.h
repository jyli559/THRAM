...
// status of DelayPipe in FU node(GPE)
...
private:
    ...
    // status of DelayPipe in each FU node(GPE)
...
public:
    ...
    const FUDelayAttr& fuDelayAttr(int id){ return _fuDelayAttr[id]; }
    ...
    int getAvailDelay(GPENode* gpeNode, DFGNode* dfgNode);
